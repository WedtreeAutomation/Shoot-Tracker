...
const Analytics = () => {
  ...
  try {
    ...
    console.log("Loaded shoot groups:", groupsData.length);
    // console.log("shoots:", groupsData);
...
const getCurrentMonthGroupedShoots = () => {
  return shootGroupsData.filter(group => {
    if (!group.ShootDate) return false;

    if (group.Status === 'cancelled') return false;

    const shootDate = new Date(group.ShootDate);
    console.log("Checking shoot date:", shootDate, "Selected month/year:", selectedMonth, selectedYear);
    return (
      shootDate.getMonth() === selectedMonth && 
      shootDate.getFullYear() === selectedYear
    );
  });
};
const currentMonthGroupedShoots = getCurrentMonthGroupedShoots();
  ...
  const totalBudget = currentMonthGroupedShoots.reduce((sum, s) => sum + (Number(s.Totalbudget) || 0), 0);
